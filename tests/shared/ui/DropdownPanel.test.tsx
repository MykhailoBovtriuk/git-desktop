...
describe('DropdownPanel', () => {
  ...
  it('align="center" applies class left-1/2 and -translate-x-1/2', () => {
    const { container } = render(<DropdownPanel align="center">content</DropdownPanel>);
  ...
  it('align="right" applies class right-0', () => {
    const { container } = render(<DropdownPanel align="right">content</DropdownPanel>);
    const div = container.firstChild as HTMLElement;
  ...
  it('custom width="w-48" applies w-48', () => {
    const { container } = render(<DropdownPanel width="w-48">content</DropdownPanel>);
    expect(container.firstChild).toHaveClass('w-48');
  });

  it('extra className merges with base classes', () => {
    const { container } = render(<DropdownPanel className="extra-class">content</DropdownPanel>);
    const div = container.firstChild as HTMLElement;
