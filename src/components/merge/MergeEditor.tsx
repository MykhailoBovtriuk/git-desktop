    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex bg-mantle border-b border-surface0 text-xs shrink-0">
        <div className={`${colHead} text-blue`}>
          ...
          <button onClick={() => setAll('ours')} className="underline text-subtext hover:text-text">
            {t('useThis')}
          </button>
        </div>
        ...
        <div className={`${colHead} text-text`}>
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setAll('both')}
              className="underline text-subtext hover:text-text"
            >
              {t('both')}
            </button>
            <button onClick={() => setAll(null)} className="underline text-subtext hover:text-text">
              {t('reset')}
            </button>
          </div>
        </div>
        ...
        {segs.map((s, idx) =>
          s.type === 'common' ? (
            <div key={idx} className="flex">
              <pre className="flex-1 px-3 py-0.5 text-subtext whitespace-pre-wrap">
                {s.lines.join('\n')}
              </pre>
              <div className="w-9 shrink-0" />
            ...
            <div key={idx} className="flex items-stretch border-y border-surface0">
              <pre
                className={`flex-1 px-3 py-1 whitespace-pre-wrap text-text ${s.choice === 'ours' || s.choice === 'both' ? 'bg-green/10' : 'bg-red/10'}`}
              >
                {s.ours.join('\n')}
              </pre>
              <button
              ...
              <div
                className={`flex-1 px-3 py-1 whitespace-pre-wrap ${s.choice ? 'bg-green/10 text-text' : 'bg-red/15'}`}
              >
                {s.choice ? (
              ...
              <pre
                className={`flex-1 px-3 py-1 whitespace-pre-wrap text-text ${s.choice === 'theirs' || s.choice === 'both' ? 'bg-green/10' : 'bg-red/10'}`}
              >
                {s.theirs.join('\n')}
              </pre>
            </div>
